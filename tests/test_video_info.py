...
VALID_YOUTUBE_URLS = ["https://youtu.be/dQw4w9WgXcQ?si=52ngrNGc_WNyEkUb",
    "https://www.youtube.com/watch?v=dQw4w9WgXcQ&list=RDdQw4w9WgXcQ", "http://youtu.be/dQw4w9WgXcQ?si=52ngrNGc_WNyEkUb",
...
def test_invalid_input_types(invalid_input) -> None:
    ...
    assert video_info.validator(invalid_input) == "Invalid link provided."
...
def test_clean_valid_urls(url: str) -> None:
    ...
    assert video_info.clean_youtube_url(url) == "Invalid YouTube link."


@pytest.mark.parametrize("invalid_input", [None, 123, [], {}, 12.34])
def test_clean_invalid_input_types(invalid_input) -> None:
    """Test that non-string inputs return error message."""
    assert video_info.clean_youtube_url(invalid_input) == "Invalid link provided."


def test_cleaning_empty_string() -> None:
    """Test that empty string is rejected."""
    assert not video_info.validator("") == "Invalid YouTube link."
...
def test_get_info_invalid_urls(url: str) -> None:
    ...
    assert video_info.get_video_info(url) == "Invalid YouTube link."
...
def test_video_unavailable() -> None:
    ...
    assert video_info.get_video_info(link) == f"Download error (video may be unavailable or private): {link}"


def test_get_info_empty_string() -> None:
    """Test that empty string is rejected."""
    assert video_info.get_video_info("") == "Invalid YouTube link."
